...
class TestDataPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        data = {
            ...
            'transaction_method': ['Credit Card', 'Debit Card', 'Credit Card', 'Debit Card', 'PayPal'],
            'transaction_timestamp': pd.date_range(start='2023-01-01', periods=5, freq='D'),
            ...
            'authentication_method': ['PIN', 'Password', 'PIN', 'Password', 'Biometric'],
    ...
    def test_split_data(self):
        ...
        X_train, X_test, y_train, y_test, X_val, y_val, X, y = split_data(self.df.copy())
    ...
    def test_select_best_features(self):
        ...
        X_train, X_test, y_train, y_test, X_val, y_val, X, y = split_data(df)
        X_train, X_test, X_val = encode_and_scale(X_train, X_test, X_val)
        X_train, X_test, X_val = select_best_features(X, X_train, y_train, X_test, X_val, print_scores=False)
    ...
    def test_data_pipeline(self):
        ...
        X_train, X_test, y_train, y_test, X_val, y_val, X, y = data_pipeline(self.df.copy(), print_scores=False)
        self.assertEqual(X_train.shape[1], 15)
        self.assertEqual(X_test.shape[1], 15)
        self.assertEqual(X_val.shape[1], 15)
        self.assertEqual(len(X_train) + len(X_test) + len(X_val), len(self.df))
